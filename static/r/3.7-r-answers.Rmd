...
speed <- read_csv("http://metricsf21.classes.ryansafner.com/Data/speeding_tickets.csv")
```
...
Using the above commands, or looking at the data should show you that these variables are `numeric` (`num`). It is also possible `R` may have read them as `integer` (`int`), a special type of `numeric` data that are whole numbers only.
...
Again we could just look at the data to verify, but here I check manually each variable:
...
# Get summary of Amount to see NAs
speed %>%
  select(Amount) %>%
  summary()
...
Note if you had not made `Female` a `factor`, and kept it numeric, it might alter the plot.

You can make `Female` a `factor` just for plotting by setting `aes(x = as.factor(Female))` inside the `aes()` layer. 
...
library(huxtable)
huxreg("Males Only" = male_age_reg,
