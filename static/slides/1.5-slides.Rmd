...
- .hi-purple[This stuff is *optional*]
...
.smaller[
From [R Studio's R Markdown Cheatsheet](https://www.rstudio.com/wp-content/uploads/2016/03/rmarkdown-cheatsheet-2.0.pdf)
]
]
]
...
.pull-left[
- .hi[Plain text] files: readable by *both* machines and humans
...
- What about output? Must you save and copy/paste to MS Word? .hi[No!]
...
Entire document is written in a single file<sup>.magenta[1]</sup> with three types of content:
...
.footnote[<sup>.magenta[1]</sup> The one exception is for managing bibliographies, this requires one additional `.bib` file!]
...
- Top of a document contains the `YAML`<sup>.magenta[1]</sup> separated by three dashes `---` above and below
...
.footnote[<sup>.magenta[1]</sup> YAML stands for "YAML Ain't Markup Language." Nerds love recursive acronyms.]
...
- You can create a .hi["chunk"] of `R` code with .hi-purple[three backticks]<sup>.magenta[1]</sup> above and below your code
- After the first pair of backticks, signify the .hi-purple[language] of the code<sup>.magenta[2]</sup> inside braces, e.g:
...
.footnote[<sup>.magenta[1]</sup> The key to the left of the #1 key on your keyboard.

<sup>.magenta[2]</sup> Yes that does mean you can use other coding languages!]
...
- .hi-purple[Name]: you can name your chunk for further reference later (not required)<sup>.magenta[1]</sup>
...
````markdown
`r ''````{r my_cool_chunk, echo=F, warning = F}
```
````
...
````markdown
`r ''````{r make-plot, echo = F, fig.height=2}
ggplot(data = mpg)+
  aes(x = displ)+
  geom_histogram()
```
...
.smaller[
- If you just want to display some code (or at least format it like code) in the middle of a sentence, .hi-purple[place between a single backtick on either side.] If I mention `tidyverse` or `gapminder`, it formats the text as `in-line code`.
...
The average GDP per capita is `` `r "\u0060r gapminder %>% mean(gdpPercap) %>% round(2)\u0060"` `` with a standard deviation of `` `r "\u0060r round(sd(gapminder$gdpPercap),2)\u0060"` `` .
...
.pull-right[
.center[
![:scale 100%](../images/rmarkdownex2.png)
]
]
---
...
- [`knitr`](https://yihui.name/knitr/)<sup>.magenta[1]</sup>, an R package that "`knit`s" your R code and markdown `.Rmd` into a `.md` file for:  
...
.footnote[<sup>.magenta[1]</sup> `knitr` also relies on the `rmarkdown` package, which will probably be installed when you first `knit.`]
---
...
.left-column[
.center[
![:scale 100%](../images/projectswitcher2.png)
]
.quitesmall[
...
.right-column[
.center[
![:scale 100%](../images/projectlist2.png)
]
.quitesmall[
...
.center[
![:scale 65%](../images/gitworkflow.png)
]
...
.center[
![:scale 70%](../images/mydropbox2.png)
.quitesmall[
...
.center[
![:scale 60%](../images/dropboxsmartsync.png)
...
- .hi[Track changes on steroids] (if MS Word’s Track Changes and Dropbox had a baby)
...
.pull-left[
.center[
![:scale 75%](../images/gitbranches.png)
]
]
.pull-right[
...
.center[
![:scale 60%](../images/basic-remote-workflow.png)
]
...
.pull-left[
.center[
![:scale 100%](../images/gitmetrics3.png)
]
]

.pull-right[
.center[
![:scale 100%](../images/gitmetrics4.png)
]
]

.center[
[github.com/ryansafner/metricsF21](http://github.com/ryansafner/metricsF21)
]
---
...
.pull-left[
.center[
![:scale 100%](../images/githubtidyverse.png)
...
.pull-right[
.center[
![:scale 100%](../images/githubgapminder.png)
...
.quitesmall[
1. Create a new repository on Github.<sup>.magenta[*]</sup>
2. Start a New R Project in R Studio (link it to the github repository<sup>.magenta[*]</sup> - [see guide](http://happygitwithr.com))
...
6. Occasionally, `stage` and `commit` changes with a description, `push` to GitHub.<sup>.magenta[*]</sup>

]

.footnote[<sup>.magenta[*]</sup> Optional and a bit advanced, remember this is _my_ workflow.]
---
