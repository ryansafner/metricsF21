...
.pull-right[
```{R, simulation-scatter, echo = F,cache = T, fig.height=4}
# Reshape sim_df
line_df <- tibble(
...
.pull-right[
```{R, ref.label="simulation-scatter", fig.height=4}
```
]
...
.quitesmall[
- Use our $\hat{\beta_1}$ and $se(\hat{\beta_1})$ to determine if we have statistically significant evidence to reject a hypothesized $\beta_1$
