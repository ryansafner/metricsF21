...
1. .hi-turquoise[Direction]: is the trend positive or negative?

2. .hi-turquoise[Form]: is the trend linear, quadratic, something else, or no pattern?

3. .hi-turquoise[Strength]: is the association strong or weak? 

4. .hi-turquoise[Outliers]: do any observations break the trends above? 
...
<sup>.magenta[‡]</sup> See today's [class notes page](/content/2.3-content#math-appendix) for example code to calculate correlation "by hand" in R using the second method.]
...
- If $X$ and $Y$ are strongly correlated, $X$ can still be .hi-purple[endogenous]!

- See [today's class notes page](/content/2.3-content#math-appendix) for more on Covariance and Correlation
...
- We do have data from a .hi-turquoise[sample] of class sizes and test scores<sup>.magenta[†]</sup>
...
- Suppose we have some data points
- We add a line
- The .hi[residual], $\hat{u_i}$ of each data point is the difference between the .hi-purple[actual] and the .hi-purple[predicted] value of $Y$ given $X$: 
...
.footnote[<sup>.magenta[†]</sup> See [next class’ notes page](/content/2.4-content) for proofs.]
---
