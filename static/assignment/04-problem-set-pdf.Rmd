...
date: "Due by Thursday, November 4, 2021"
output:
  pdf_document:
...
9. Install and load the `wooldridge` package. This package contains datasets used in Jeffrey Wooldridge's *Introductory Econometrics: A Modern Approach* (the textbook that I used in *my* econometrics classes years ago!).
...
a. Make a correlation table for our variables listed above. 
...
```{r, echo=T, message = F, warning = F, fig.align = "center", fig.height = 3}
library(ggdag)
...
f. Estimate the causal effect by running the appropriate regression. 
...
g. We saw some effect between `faminc` and `cigprice`. Perhaps there are unobserved factors (such as the economy's performance) that affect both. Add an unobserved factor `u1` to your `dagitty` model.
...
c. How much would someone who is 5'10" (70 in) be predicted to earn per hour, according to the model?
...
i. Quantify how much multicollinearity affects the variance of the OLS estimates on both heights. Hint: You'll need the `vif` command from the `car` package.
